...
namespace codi {
  ...
  namespace ComputationTraits {
    ...
    ///
    /// Has to be used in the codi::ComputationTraits namespace.
    #define CODI_CREATE_ADJOINT_CONVERSION(OuterType, InnerType, RType, conversion) \
      template<>                                                                    \
      struct AdjointConversionImpl<OuterType, InnerType> {                          \
        public:                                                                     \
    ...
    #define CODI_CREATE_UPDATE(LhsType, RhsType, up)           \
      template<>                                               \
      struct UpdateImpl<LhsType, RhsType> {                    \
        public:                                                \
          using Lhs = LhsType;                                 \
          using Rhs = RhsType;                                 \
    ...
    struct AdjointConversionImpl<Type, Type> {
      public:
        using Outer = Type;
        using Inner = Type;
    ...
    template<typename Type>
    struct AdjointConversionImpl<Type, std::complex<Type>> {
      public:
        using Outer = Type;
    ...
    template<typename T>
    struct TransposeImpl<T, typename std::enable_if<std::is_floating_point<T>::value>::type> {
      public:
        using Jacobian = T;
        using Return = T;
    ...
    struct TransposeImpl<std::complex<Inner>> {
      public:
    ...
    template<typename Inner>
    struct UpdateImpl<Inner, std::complex<Inner>> {
      public:
        using Lhs = Inner;
