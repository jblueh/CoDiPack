...
#include "../../tapes/tagging/tagTape.hpp"
...
namespace codi {
   ...
   * preaccumulation is finisched the original tag is restored on the tape, the inputs
   * and the outputs.
   */
  template<typename T_Type>
  struct PreaccumulationHelper<T_Type, typename enable_if_same<
      typename T_Type::Tape,
      TagTape<typename T_Type::Real, typename T_Type::Tape::Tag>>::type > {
    public:

      using Type = CODI_DD(T_Type, CODI_DEFAULT_LHS_EXPRESSION);

      using Tape = CODI_DD(typename Type::Tape, CODI_T(TagTape<Type, int>));
