...
namespace codi {
  ...
  struct PrimalValueBaseTape : public CommonTapeImplementation<T_TapeTypes, T_Impl>,
    ...
    public:
      ...
      using StmtCallArgs =
          CODI_DD(
            CODI_T(typename std::conditional<LinearIndexHandling,
              LinearStatementCallDefaultArguments<Identifier>,
      ...
      using StmtDynamicDataEntry = StatementDynamicDataEntry<Real, Identifier, StmtCallArgs, DynamicSizeData,
                                   LinearIndexHandling>;

/// Define the default arguments for evaluate handle calls.
#define STMT_ARGS typename StmtCallArgs::PackHelper packHelper, size_t& __restrict__ curDynamicSizePos, char* const __restrict__ dynamicSizeValues
...
#define STMT_ARGS_PACK StmtCallArgs{packHelper, curDynamicSizePos, dynamicSizeValues}
...
#define STMT_ARGS_FORWARD packHelper, curDynamicSizePos, dynamicSizeValues
    ...
    protected:
      ...
      CODI_INLINE bool storeArgumentData(ExpressionInterface<RhsType, Rhs> const& rhs, StmtDynamicDataEntry& dynamicPointers) {
        ...
        if (CODI_ENABLE_CHECK(Config::CheckEmptyStatements, 0 != activeArguments)) {
          ...
          dynamicPointers = StmtDynamicDataEntry::store(dynamicSizeData, StatementSizes::create<Expr>(), activeArguments);
          pushStatement.eval(rhs.cast(), dynamicPointers, passiveArguments, idPos, constantPos);

          StmtFixedDataEntry::store(fixedSizeData, (Config::ArgumentSize)passiveArguments, StatementEvaluator::template createHandle<Impl, Impl, Expr>());
    ...
    protected:
      ...
      CODI_INLINE Real internalRegisterInput(LhsExpressionInterface<Real, Gradient, Impl, Lhs>& value,
        ...
        if (TapeTypes::IsLinearIndexHandler) {
          StmtFixedDataEntry::store(fixedSizeData, Config::StatementInputTag, StatementEvaluator::template createHandle<Impl, Impl, AssignExpression<Lhs, Lhs>>());
        }
    ...
    public:
      ...
      void storeManual(Real const& lhsValue, Identifier& lhsIndex, Config::ArgumentSize const& size) {
        ...
        size_t const activeArguments = 0; // Zero active arguments means, that size number of reals are reserved which we
                                    // use to push the Jacobians.
        ...
        StmtFixedDataEntry::store(fixedSizeData, (Config::ArgumentSize)size, PrimalValueBaseTape::jacobianExpressions[size]);
    ...
    public:
      ...
      struct StatementCallGeneratorBase {
        public:
          ...
          template<size_t pos>
          using ExtractExpr = ArrayAccessExpression<LhsReal, pos, StaticRhs>; ///< Extract expressions for aggregated types.
  ...
  struct JacobianStatementGenerator :
    ...
    public:
      ...
      using TapeImpl = CODI_DD(T_TapeImpl, CODI_T(PrimalValueBaseTape<
          PrimalValueTapeTypes<double, double, IndexManagerInterface<int>,
          StatementEvaluatorInterface, DefaultChunkedData>, void>)); ///< PrimalValueBaseTape.
